...
def _normalize_receipt_type(promotion_receipt: Dict[str, Any]) -> str:
    return str(
        promotion_receipt.get("receipt_type")
        or promotion_receipt.get("artifact_type")
        or ""
    )
...
def aggregate_pattern(
    ...
    target_core = str(
        promotion_artifact.get("target_core")
        or promotion_artifact.get("target_child_core")
        or "unknown"
    )
    ...
    receipt_type = _normalize_receipt_type(promotion_receipt)
    if receipt_type != REGISTRY["accepted_receipt_type"]:
        return {
            "status": "failed",
            "receipt": build_rejection_receipt(
                artifact_type=promotion_artifact["artifact_type"],
    ...
    receipt_target_core = str(
        promotion_receipt.get("target_core")
        or promotion_receipt.get("target_child_core")
        or ""
    )

    if (
        promotion_receipt.get("artifact_type") != promotion_artifact["artifact_type"]
        or receipt_target_core != target_core
    ...
    decision = str(promotion_artifact.get("decision", "")).strip().lower()
    promotion_status = str(promotion_artifact.get("promotion_status", "")).strip().lower()

    if decision != "promoted" and promotion_status != "promoted":
        return {
            "status": "failed",
            "receipt": build_rejection_receipt(
                artifact_type=promotion_artifact["artifact_type"],
    ...
    artifact = {
        ...
        "promoted_record_count": promotion_artifact.get("promoted_record_count", recurrence_count),
        "recurrence_count": recurrence_count,
        "temporal_span_days": temporal_span_days,
        ...
        "upstream_receipt_type": receipt_type,
    }
