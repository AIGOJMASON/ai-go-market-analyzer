# AI_GO/EXTERNAL_MEMORY/output_merge/output_merge_runtime.py
...
def _validate_inputs(
    ...
    receipt_type = str(return_receipt.get("artifact_type") or return_receipt.get("receipt_type") or "")
    if receipt_type != OUTPUT_MERGE_REGISTRY["accepted_return_receipt_type"]:
    ...
    if not isinstance(advisory_summary, dict) or advisory_summary.get("state") != "present":
        return False, "missing_required_advisory_fields", "advisory_summary_state_missing"

    if not isinstance(memory_context_panel, dict) or memory_context_panel.get("state") != "present":
