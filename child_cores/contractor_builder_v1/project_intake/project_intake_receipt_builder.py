...
RECEIPT_VERSION = "northstar_intake_receipt_v1"
RECEIPT_ROOT = state_root() / "contractor_builder_v1" / "receipts" / "project_intake"
...
def _classification_block() -> Dict[str, Any]:
    return {
        "persistence_type": "contractor_project_intake_receipt",
        "mutation_class": "artifact_record",
...
def _authority_metadata(
    *,
    ...
    return {
        ...
        "layer": "project_intake.project_intake_receipt_builder",
        "project_id": _safe_str(project_id),
        "event_type": _safe_str(event_type),
    }


def build_project_intake_receipt(
    *,
    event_type: str,
    project_id: str,
    artifact_path: str,
    ...
    receipt_id = f"receipt-project_intake-{clean_event_type}-{timestamp}-{uuid4().hex[:8]}"
    ...
    return {
        ...
        "authority_metadata": _authority_metadata(
            operation="build_project_intake_receipt",
            project_id=clean_project_id,
            event_type=clean_event_type,
        ),
        "sealed": True,
...
def write_project_intake_receipt(receipt: Dict[str, Any]) -> Path:
    ...
    payload["authority_metadata"] = _authority_metadata(
        operation="write_project_intake_receipt",
