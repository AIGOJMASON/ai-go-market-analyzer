...
def _payload():
    return {
        ...
        "case_panel": {"case_id": "probe-002", "title": "Probe case"},
...
def _watcher_receipt(status: str = "passed"):
    return {
...
def main():
    ...
    record = build_pre_interface_smi_record(_payload(), _watcher_receipt())
    if record["artifact_type"] == "pre_interface_smi_record" and record["recommendation_count"] == 2:
        passed += 1
        results.append({"case": "case_01_valid_record_created", "status": "passed"})
    else:
        failed += 1
