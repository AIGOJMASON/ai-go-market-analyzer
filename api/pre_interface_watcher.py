...
PRE_INTERFACE_WATCHER_MUTATION_CLASS = "pre_interface_watcher_persistence"
PRE_INTERFACE_WATCHER_PERSISTENCE_TYPE = "filesystem"
PRE_INTERFACE_WATCHER_ADVISORY_ONLY = False
...
def _classification_block() -> Dict[str, Any]:
    return {
        "mutation_class": PRE_INTERFACE_WATCHER_MUTATION_CLASS,
        "persistence_type": PRE_INTERFACE_WATCHER_PERSISTENCE_TYPE,
        "advisory_only": PRE_INTERFACE_WATCHER_ADVISORY_ONLY,
...
def _validate_required_panels(payload: Dict[str, Any], failures: List[str]) -> None:
    for key in REQUIRED_PANELS:
        ...
        elif not _is_panel_dict(payload[key]):
    ...
    return {
        ...
        "checks": {
            "required_sections": all(
                not item.startswith("missing_required_panel")
                and not item.startswith("invalid_required_panel_shape")
                for item in failures
            ),
            "advisory_only": "execution_allowed_must_be_false" not in failures
            and "mode_must_be_advisory" not in failures,
            "internal_leakage_absent": not any(
                item.startswith("forbidden_key_present") for item in failures
            ),
...
def persist_pre_interface_watcher_receipt(receipt: Dict[str, Any]) -> str:
    receipt.setdefault("classification", _classification_block())
    receipt.setdefault("authority", _authority_block())

    path = _receipt_dir() / f"{receipt['receipt_id']}.json"
    governed_write_json(
        path=path,
        payload=receipt,
        mutation_class=PRE_INTERFACE_WATCHER_MUTATION_CLASS,
