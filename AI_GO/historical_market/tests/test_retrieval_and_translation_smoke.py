...
class TestRetrievalAndTranslationSmoke(unittest.TestCase):
    def setUp(self) -> None:
        self._tmpdir = tempfile.mkdtemp(prefix="ai_go_retrieval_translation_")
        ...
        self.runtime = HistoricalQueryRuntime(paths=self.paths)
        self.adapter = OperatorTranslationAdapter()
        ...
        self.pipeline.process_event(
            ...
            horizon_bars=3,
            outcome_notes="Retrieval smoke test.",
        )

    def tearDown(self) -> None:
        shutil.rmtree(self._tmpdir, ignore_errors=True)

    def test_retrieval_and_translation(self) -> None:
        summary = self.runtime.summarize_setup_outcomes(setup_type="dip_rebound")
        self.assertEqual(summary["status"], "ok")
        self.assertEqual(summary["pattern_count"], 1)
        self.assertEqual(summary["outcome_counts"]["follow_through"], 1)

        panel = self.adapter.build_setup_history_panel(
            setup_type="dip_rebound",
            setup_summary=summary,
        )
        self.assertEqual(panel["panel_type"], "setup_history")
        self.assertEqual(panel["setup_type"], "dip_rebound")
        self.assertIn("historical_posture", panel)
        self.assertIn("operator_summary", panel)

        event_package = self.runtime.get_event_package(event_id="hist-xle-20260403-01")
        self.assertEqual(event_package.status, "ok")
        self.assertEqual(event_package.match_count, 2)

        event_panel = self.adapter.build_event_package_panel(
            event_id="hist-xle-20260403-01",
            event_records=event_package.matches,
        )
        self.assertEqual(event_panel["panel_type"], "event_package")
        self.assertEqual(event_panel["setup_type"], "dip_rebound")
        self.assertEqual(event_panel["outcome_label"], "follow_through")