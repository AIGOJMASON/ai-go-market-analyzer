...
CONTINUITY_MUTATION_CLASS = "child_core_continuity_memory_persistence"
CONTINUITY_PERSISTENCE_TYPE = "in_memory_continuity_store"
CONTINUITY_ADVISORY_ONLY = False
...
def _validate_receipt_structure(receipt: Any) -> Tuple[bool, Optional[str], str]:
    ...
    bad_keys = [
        key
        for key in sorted(REQUIRED_RECEIPT_KEYS - LINEAGE_KEYS)
...
def _authority_metadata(receipt: Dict[str, Any], continuity_key: str) -> Dict[str, Any]:
    return {
        "authority_id": "core_child_flow_continuity_mutation",
        "operation": "process_continuity_mutation",
        "target_core": receipt.get("target_core"),
        "continuity_scope": receipt.get("continuity_scope"),
...
def _classification() -> Dict[str, Any]:
    return {
        "mutation_class": CONTINUITY_MUTATION_CLASS,
        "persistence_type": CONTINUITY_PERSISTENCE_TYPE,
        "advisory_only": CONTINUITY_ADVISORY_ONLY,
...
def _persist_continuity_record(continuity_key: str, record: Dict[str, Any]) -> None:
    mutation_class = CONTINUITY_MUTATION_CLASS
    persistence_type = CONTINUITY_PERSISTENCE_TYPE
    advisory_only = CONTINUITY_ADVISORY_ONLY
    authority_metadata = record.get("authority_metadata", {})

    governed_record = dict(record)
    governed_record.setdefault("classification", _classification())
    governed_record.setdefault("authority_metadata", authority_metadata)
    governed_record.setdefault("sealed", True)

    store.records[continuity_key] = governed_record
...
def process_continuity_mutation(receipt: dict):
    ...
    authority_metadata = _authority_metadata(receipt, continuity_key)
    ...
    _persist_continuity_record(continuity_key, record)
